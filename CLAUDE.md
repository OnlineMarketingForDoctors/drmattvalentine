...
rather than `href` attributes — `Nav.astro`'s `links` and `src/data/pages.ts`
— so a find-and-replace over `href="` will miss them.
...
`src/data/pages.ts` is the single source for the `/sitemap/` page and
`/llms.txt` (generated by `src/pages/llms.txt.ts`). **Add a page there when
you add one to `src/pages/`**, or it will be missing from both.

The thank-you pages are deliberately absent from that list.
